...
const GenealogistDashboard = ({ onLogout }) => {
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      try {
        ...
        const res = await axios.get('https://honduras-archive-ver-1-1.onrender.com/api/auth/dashboard', {
  ...
  const handleLogout = async () => {
    try {
      ...
      await axios.post('https://honduras-archive-ver-1-1.onrender.com/api/auth/logout',
  ...
  const removeBookmark = async (recordId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`https://honduras-archive.onrender-ver-1-1.com/api/auth/activity/bookmark/${recordId}`,
        {}, { headers: { 'x-auth-token': token } }
      );
      setData(prev => ({
        ...prev,
  ...
  const saveNote = async (recordId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`https://honduras-archive.onrender-ver-1-1.com/api/auth/activity/note/${recordId}`,
        { text: noteText },
        { headers: { 'x-auth-token': token } }
      );
      setData(prev => ({
